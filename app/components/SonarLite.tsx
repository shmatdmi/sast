...
import { AlertIcon, CheckIcon, CodeIcon, DownloadIcon } from "./icons";
...
type Props = { sources: SonarSource[]; result: SonarResult | null; onExport: () => void };

export default function SonarLite({ sources, result, onExport }: Props) {
  ...
  return <section className="sonar-panel" id="sonar"><div className="panel-header"><div><span className="panel-dot sonar-dot" /><div><h2>Sonar Lite</h2><p>Упрощённый локальный анализ качества кода</p></div></div>{result && <button className="export-button" onClick={onExport}><DownloadIcon size={16} />JSON</button>}</div>
